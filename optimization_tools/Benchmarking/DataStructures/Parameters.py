...
class DiscreteParameter(ListBasedParameter):
    def step(self):
        ...
        self.current_idx = choice([self.current_idx - 1, self.current_idx + 1])
...
def getCompressionParameter(
    current_value: str,
    compression_types: list[str] = ["none", "zlib", "lz4", "lzma", "zstd"],
    ...
    return CategoricalParameter(
        "Compression Type", compression_types, current_idx=current_idx
    )
...
def getPageSizeParameter(current_value) -> DiscreteParameter:
    ...
    return DiscreteParameter(
        "Page Size", page_sizes, value_names=value_names, current_idx=current_idx
    )
...
def getClusterSizeParameter(current_value) -> DiscreteParameter:
    ...
    return DiscreteParameter(
        "Cluster Size", cluster_sizes, value_names=value_names, current_idx=current_idx
    )
